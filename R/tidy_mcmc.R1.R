...
get_sport <- function(sport) {
  ...
  load(file.path(mcmc_dir, paste0(sport, "_paper_teamHFA.R1.RData")))
  out <- data.frame(
...
params <- lapply(dat, function(x) { return(x[["out"]]) } ) %>% 
  bind_rows()
# save the params
save(params, file = file.path(root, "data", "params.R1.rda"), compress = "xz")
...
save(tidy_thetas, file = file.path(root, "data", "tidy_thetas.R1.rda"), compress = "xz")
...
save(tidy_alphas, file = file.path(root, "data", "tidy_alphas.R1.rda"), compress = "xz")  
