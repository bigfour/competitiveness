...
for (league in c("nfl","mlb","nhl","nba")){print(league)
  ...
  load(paste0(mcmc_dir,"/", league,"_paper_teamHFA.logit.R2.RData"))
...
save(postPred, file = paste0(root, "/data/postPred.team.r2.RData"))
save(yList, file = paste0(root, "/data/yList.team.r2.RData"))
...
save(postPred.constant, file = paste0(root, "/data/postPred.constant.r2.RData"))
...
library(ggjoy)
load(file.path(data_raw, "bigfour.rda"))
load(file.path(root, "data/postPred.constant.r2.RData"))
load(file.path(root, "data/postPred.team.r2.RData"))
postPred.team <- postPred
load(file.path(root, "data/yList.team.r2.RData"))
...
bigfour %>% filter(sport == "nfl") %>% group_by(p_home) %>% count() %>% ungroup() %>% arrange(-n) %>% slice(1:5)
bigfour %>% filter(sport == "nfl") %>% filter(p_home > 0.63, p_home < 0.64)
bigfour %>% filter(sport == "nfl") %>% group_by(ml_home) %>% count() %>% ungroup() %>% arrange(-n) 
...
team.ppd <- df.all %>% mutate(diff.constant = y.tilde.constant - y.obs, 
  ...
  mutate(type = ifelse(type == "ave.diff.constant", "CHA", "IHA")) %>% 
  rename(Model = type) 

save(team.ppd, file = paste0(root, "/data/postPred.teamplot.RData"))

team.ppd %>% 
          ggplot(aes(x = hteam.obs, y = Average_difference, pch = Model, colour = Model)) + 
  geom_point() + xlab("Team") + geom_hline(aes(yintercept = 0)) +  coord_flip() + 
         facet_wrap(~toupper(sport), scales = "free_y") + theme_gray(12) + ylab("Difference (log odds scale)")

ggsave(plot = p2, width = 6, height = 8, filename = "~/Dropbox/Compete/figure/ppd-model.pdf")
