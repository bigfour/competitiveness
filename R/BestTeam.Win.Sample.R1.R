...
## Step 7
tauGame.league <- params %>% mutate(tauGame = sqrt(tauGame)) %>% 
  select(tauGame) 
tauGame.sample <- sample(tauGame.league$tauGame, 1)
epsilon <- rnorm(1, 0, tauGame.sample)
...
save(cdf.all, file = file.path(root, "data", "bestwin.R1.rda"))  
