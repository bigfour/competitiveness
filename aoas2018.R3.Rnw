...
\begin{figure}[h]
\includegraphics[angle=90]{figure/scales_simulated.png}
\caption{Standard deviations of team strength estimates over time, for each sport $q$. Each line corresponds to standard deviations from different home advantage scenarios: no home advantage (`None`), MLBs home advantage (`MLB`), NBAs home advantage (`NBA`), and the observed standard deviations from the initial model fits (`Observed`).  \label{fig:simscales}}
\end{figure}
