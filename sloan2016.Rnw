...
\ULCornerWallPaper{1}{gfx/Top.png}
\LLCornerWallPaper{1}{gfx/Bottom.png}
...
An attractive property of this specification is that past and future season performances are incorporated into season-specific measurements of team quality. Perhaps as a result, \cite{koopmeiners2012comparison} identified stronger fits when comparing state-space models as specified above with BTM's fit separately within each season. For additional and related resources, see \cite{tutz2015extended}, \cite{koopmeiners2012comparison}, \cite{matthews2005improving}, \cite{wolfson2015s} and \cite{owen2011dynamic}. 
...
tidy_thetas %>%
  ...
  summarize(N = n(), 
            min = min(theta),
            CIL = quantile(theta, probs = 0.05),
            Q1 = quantile(theta, probs = 0.25),
            mean = mean(theta), 
            Q3 = quantile(theta, probs = 0.75),
            CIR = quantile(theta, probs = 0.95),
            max = max(theta),
            sd = sd(theta)) %>%
...
library(ggplot2)
tidy_thetas <- mutate(tidy_thetas, time_val = 
        ifelse(sport == "NFL"|sport == "MLB", time_val + 1, time_val))
spag <- ggplot(data = tidy_thetas, 
...
sport1 <- eval(sport)
sportcolors <- subset(tidy_thetas, sport == sport1)
...
datthetas3 <- inner_join(tidy_thetas, sportcolors, by = c("name" = "name"))
datthetas.null <- select(datthetas3, -division)
datthetas.null <- arrange(datthetas.null, time_val)


##### Generate plots
lim <- max(abs(datthetas.null$theta))
gg <- ggplot()  + ggtitle(paste("Team strengths over time,", sport)) + xlab("Year")  + 
                            ylab("Log-odds of a win")
gg <- gg + geom_path(data = datthetas.null, colour = "grey", 
...
for (i in 1:nrow(sportcolors)){
    gg <- gg + geom_line(data = filter(datthetas3,name==sportcolors$name[i]),aes(x = time_val, y = theta),linetype=1,lwd=1.5,colour=sportcolors$primary[i]) 
    gg <- gg + geom_line(data = filter(datthetas3,name==sportcolors$name[i]),aes(x = time_val, y = theta),linetype=2,lwd=1.5,colour=sportcolors$secondary[i]) 
  }
gg1 <- gg +
...
params %>%
  ...
  rename(`Seasonal autoregressive estimate` = gamma_s, `Weekly autoregressive estimate` = gamma_w) %>%
...
library(ggplot2)
datthetas.null <- tidy_alphas %>%
  mutate(sport = toupper(sport)) %>%
  group_by(sport) %>%
  arrange(alpha.team.overall) %>%
  mutate(rank.within = 1:n()) 
datthetas.empty <- datthetas.null %>% ungroup() %>% select(-sport)

gg <- ggplot(datthetas.null)  + ggtitle("Who has the best home advantage? Increased log-odds of winning at home, by team") 
gg1 <- gg + geom_point(data = datthetas.empty, colour = "grey", 
                       aes(y=alpha.team.overall, x=rank.within, group = team)) + 
  geom_errorbar(data = datthetas.empty, colour = "grey", 
                aes(x=rank.within, ymax = alpha.team.upper, 
                    ymin=alpha.team.lower, group = team), width=0.2) 

for (i in 1:nrow(datthetas.null)){
  df.temp <- slice(ungroup(datthetas.null), i)
...
Given the breadth of subjects that our work touches on, we conclude with a list of practical recommendations for league officials, team personnel, and analytically-inclined fans.
...
Understanding this underlying randomness and the proper evaluation of team talent is crucial to decision makers. At critical moments in a team's evolution, such as the trade deadline, free agency, or in the choice to fire a coach, we recommend that team officials look past wins and losses to better understand team strength in the context of their league. As one easy example, it is insufficient to evaluate a baseball or hockey team based on their performance in the postseason alone, given that so many of those contests are near 50-50 outcomes. 

\item We argue that it is substantially more beneficial to play at home in the NBA, and to a lesser extent the NFL, than it is in MLB and the NHL. As a result, NBA and NFL team decision makers should be cognizant of such factors when allocating resources at the end of the regular season to secure additional home games in the postseason. In addition, an awareness of the relative difficulty of playing in Denver and Utah could lend itself to different line-ups and distribution of playing time in the NBA. 

\item Fans hoping that their teams will rebound from an off-year may be interested in our consistency findings within each league. From one season to the next, team strengths are the most consistent in the NFL and the least consistent in the NHL, and from one week to the next, team strengths are the most consistent in MLB and the least consistent in the NBA. In addition, there appear to be league level evolutionary traits in MLB (where team strength appears to be a random walk within a season) and NHL (where team strengths may converge over time) that potentially tie into league policies (see Section \ref{sect::PostEst}). 

\item The NFL and NBA stand out as leagues showing the least amount of parity, where parity is defined as equivalent underlying team quality. Under the hypothesis that increased parity leads to higher revenue, perhaps those leagues should explore options for making an increased fraction of games more competitive. 

\item Leagues may also want to consider the equity of their standards for postseason qualification. For example, in the NFL (Figure \ref{fig:spaghetti-nfl} in the Appendix), there are multiple examples of seasons in which one division's teams were all worse than average, such as 2015 (the AFC South) or 2010 (NFC West). In these instances, despite none of these franchises being talented enough to deserve a postseason berth, the NFL's division-heavy schedule and automatic qualification for the divisional winner have awarded not only playoff opportunities to less deserving teams, but also home games. 

\item Finally, our findings with respect to league-level differences in team strengths call into question the equivalence of each league's postseason format. For example, the NBA begins its playoffs with a seven-game series between a No. 1 seed and its No. 8 seed, while MLB starts its postseason with a one-game play-in game and a $five$-game series between a No. 1 seed and a No. 4 seed. This set-up is markedly inequitable. The best team in the NBA will almost always win such a series, while the best MLB team will lose nearly as often as it will win, with the latter outcome having less to do with talent and more to do with the inherent noise embedded in the sport of baseball. 
...
\begin{eqnarray}
logit(p_{(q,s,k)}) \sim N(\theta_{(q,s,k)} X_{q,s,k} + \alpha_{q_0} J_{g_{q,s,k}} + \alpha_{q} Z_{q,s,k}, \tau^{2}_{q,game} I_{g_{(q,s,k)}}) \,, \label{model.full.appendix}
\end{eqnarray}

where $\theta_{(q,s,k)}$ is a vector of length $t_{q}$ (the number of teams in league $q$) containing the team strength parameters in season $s$ during week $k$, $\alpha_{q_0}$ is the overall home field advantage parameter for sports league $q$, and $\alpha_{q}$ is a vector of length $t^{\star}_{q}$ containing \textit{franchise}-specific home field advantage parameters for league $q$ that do not vary over time (i.e. HFA is assumed to be constant for a team over weeks and seasons). Note that $t^{\star}_{q}$, the number of home venues in league $q$, is greater than $t_{q}$, the number of teams in league $q$, due to franchise relocation.\footnote{There were two franchise relocations in our data, Seattle to Oklahoma City (NBA) and Atlanta to Winnipeg (NHL).} $X_{q,s,k}$ and $Z_{q,s,k}$ contain $g_{(q,s,k)}$---the number of games in league $q$ during week $k$ of season $s$---rows and $t_{q}$ and $t^{\star}_{q}$ columns, respectively.   The matrix $X_{q,s,k}$ contains the values $\{1, 0, -1\}$ where for a given row (i.e. one game) the value of $i^{th}$ column in that row is a 1/-1 if the $i^{th}$ team played at home/away in the given game and 0 otherwise.  $Z_{q,s,k}$ is a matrix containing a 1 in the $i^{th}$ column if the $i^{th}$ team played the game corresponding to that row at home and 0 otherwise. Finally, $\tau^{2}_{q,game} = 1 / \sigma^{2}_{q,game}$ is the precision, $ J_{g_{q,s,k}}$ is a column vector of length $g_{q,s,k}$ containing all 1's, and $I_{g_{(q,s,k)}}$ is an identity matrix with dimension ${g_{(q,s,k)}} \times {g_{(q,s,k)}}$.
...
$$
\theta_{(q,s+1,1)} | \gamma_{q,seas}, \theta_{q,s,g_{q,s,.}}, \tau^{2}_{q,seas},  \sim N (\gamma_{q,seas} \theta_{(q,s,g_{q,s,.})},(\tau^{2}_{q,seas})I_{t_{q}})
$$
and 
$$
\theta_{(q,s,k+1)} | \gamma_{q,week}, \theta_{q,s,k}, \tau^{2}_{q,week},  \sim N (\gamma_{q,week} \theta_{(q,s,k)},(\tau^{2}_{q,week})I_{t_{q}})
$$
