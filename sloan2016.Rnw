...
\item The NFL and NBA stand out as leagues showing the least amount of parity, where parity is defined as the distribution of underlying team quality. Under the hypothesis that increased parity leads to higher revenue, perhaps those leagues would do well to explore options for making an increased fraction of games more competitive. 

\item Leagues may also want to consider the equity of their standards for postseason qualification. For example, in the NFL (Figure \ref{spaghetti-nfl} in the Appendix), there are multiple seasons, such as the AFC South in 2015 or the NFC West in 2010, in which one division's teams were all worse than average. In these instances, despite none of these franchises being talented enough to deserve a postseason berth, the NFL's division-heavy schedule and automatic qualification for the divisional winner have awarded not only playoff opportunities to less deserving teams, but also home games. 

\item Finally, our findings with respect to league-level differences in team strengths raise into question the equivalence of each league's postseason format. For example, the NBA begins its playoffs with a seven-game series between a No. 1 seed and its No. 8 seed, while the MLB starts its postseason with a one-game play-in game and a $five$-game series between a No. 1 seed and a No. 4 seed. This set-up is markedly inequitable. The best team in the NBA will almost always win such a series, while the best MLB team will still often lose, with the latter outcome having less to do with talent and more to do with the inherent noise in the sport of baseball. 
