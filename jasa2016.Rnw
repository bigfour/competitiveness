...
\noindent In Model CFA, matrices ${\bf p}_{(q,s,k)}$, $\mathbf{X}_{(q,s,k)}$, $\mathbf{J}_{g_{(q,s,k)}}$, and $\mathbf{I}_{g_{(q,s,k)}}$ are specified identically to Model IFA. As a result, for a game between home team $i$ and away team $j$ during week $k$ of season $s$, $E[\text{logit}(p_{(q,s,k) ij})] = \theta_{(q,s,k) i} - \theta_{(q, s, k) j} + \alpha_{q_0}$. 
...
$$
\theta_{(q,s+1,1)} |  {\bf \theta}_{q,s,K_q}, \gamma_{q,season}, \sigma^{2}_{q,season} \sim N (\gamma_{q, season}\mathbf{\theta}_{(q,s,K_q)}, \sigma^{2}_{q,season}\mathbf{I}_{t_{q}}) \text{ for $s \in 2, \ldots, S_q$, and }
$$

$$
\theta_{(q,s,k+1)} | \mathbf{\theta}_{(q,s,k)}, \gamma_{q,week}, \sigma^{2}_{q,week}  \sim N (\gamma_{q, week}\mathbf{\theta}_{(q,s,k)},\sigma^{2}_{q,week}\mathbf{I}_{t_{q}}) \text{ for $s \in 1, \ldots, S_q$ and $k = 2, \ldots, K_q$.}
$$

In this specificaiton, $\gamma_{q,week}$ is the autoregressive parameter from week-to-week, $\gamma_{q,season}$ is the autoregressive parameter from season-to-season, with $\mathbf{I}_{t_{q}}$ identity matrix of dimension $t_{q} \times t_{q}$.
...
Finally, letting $\tau^{2}_{q,season} = 1/\sigma^{2}_{q,season}$, $\tau^{2}_{q,week} = 1/\sigma^{2}_{q,week}$, and $\tau^{2}_{q,\alpha} = 1/\sigma^{2}_{q,\alpha}$ we assume the following prior distributions: 
\begin{align*}
...
Posterior draws of $\sigma_{q, game}$ suggest that the highest game-level errors in our log-odds probability estimates occur in the NBA (median posterior draw of $\sigma_{NBA, game}$ = 0.273), followed in order by the NFL, MLB, and the NHL. Interestingly, although Figure \ref{fig:spaghetti} identifies that the average talent gap between teams is smallest in MLB, $\sigma_{MLB, game} \approx 2 \times \sigma_{NHL, game}$. We posit that this additional game-level error in MLB is a function of the league's pitching match-ups, in which teams rotate through a handful of starting pitchers of varying calibers. 
...
Such distinctions have plausible impacts on league standings. An NBA team with a typical home advantage can expect to win 62.0\% of home games against a like-caliber opponent. Yet for Brooklyn, the corresponding figure is \Sexpr{100*league.low}\%, while for Denver, it is \Sexpr{100*league.high}\%. Across 41 games (the number each team plays at home), this implies that Denver's home advantage is worth an extra \Sexpr{round(41*(league.high-62.0), 2)} wins in a single season, relative to a league average team. Compared to Brooklyn, Denver's home advantage is worth an estimated \Sexpr{round(41*(league.high-league.low), 2)} wins per year. 
...
For each $q$, we simulated with $n_{sim} = 1000$. Additionally, to remove the effect of each league's HA on simulated probabilities, we repeated the process using after fixing $\tilde{\alpha}_{q_0} = 0$ for each league, reflecting the game's probability were it to be played at a neutral site.
