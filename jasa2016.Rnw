...
\greg{Wait, is this backwards.  Aren't the $\tau$'s precision?  That would make the MLB the most varible.} \mike{The $\tau$'s are the standard deviation/variance, and the $\sigma$'s are the precision (you can check the JAGS model). This is perhaps one reason to tighten up our notation in the JAGS model and the tidy_betas/tidy_alphas/tidy_mcmc files.}
