...
Mathematically, if team $i$'s line money line is $\ell_i$ against team $j$ (with money line $\ell_j$), then the boundary win probability for that team, $p_i(\ell_i)$, is given by:
$$
