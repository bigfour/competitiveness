...
\greg{I think we need to say after the model $i,j = 1, \cdots, t_{q}$, $q = 1,\cdots, 4$, etc.} \mike{I added the space for everything but $s$ and $k$...which we may still want to add in.}\\
...
for $s = 1, \cdots, 11$ \greg{however, many seasons we have}
