...
bigfour %>%
  ...
  group_by(sport) %>%
  summarise(earliest = as.character(min(gameDate)), 
  ...
  xtable(caption = "Summary of cross-sport data. Note that we have about 99.6% coverage (betting odds for every game) across all four major sports during the 2005--2014 regular seasons.", 
