...
Next, let $\hat{\bar{\theta_{q,s}}}_{\|r\|}$ be the $r^{th}$ ranked team strength in sport $q$ in season $s$. For example, the $\hat{\bar{\theta_{MLB,2005}}}_{\|1\|} = 0.499$ represents the average posterior draw of team strength for 2005 New York Yankees, which ranks first in MLB for that season. Teams ranked first in each season in each sport were matched-up against teams ranked No. 2 - No. 8. To simulate a 'best of $n$' series between the first ranked team ($r_{1, q, s}$) and a lower ranked team ($r_{2, q, s}, \ldots r_{8, q, s}$), the following procedure was used. 

\begin{enumerate}
\item Draw $n$ $\theta_{r,q,s}$
