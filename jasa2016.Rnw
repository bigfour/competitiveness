...
Let $\widehat{\theta}_{q,s,k,i}$ be the posterior distribution of the team estimated team strength of $i$ at week $k$. Teams in league $q$ at season $s$ were ranked based on $\bar{\widehat{\theta}}_{q,s} = (\widehat{\theta}_{q,s,(k*-4)} + \widehat{\theta}_{q,s,(k*-3)} + \widehat{\theta}_{q,s,(k*-2)} + \widehat{\theta}_{q,s,(k*-1)}+ \widehat{\theta}_{q,s,(k)} \})/5, the average posterior team strength over the last five weeks of the regular season. These draws are meant to roughly reflect team quality entering the postseason; five weeks are used in place of one week to account for the fact that in many instances, top performing teams rest top players in the final few games, thereby likely lowering their perceived team quality.

Next, let $\widehat{\bar{\theta}}_{q,s, \|r\|}$ be the $r^{th}$ ranked team strength in sport $q$ in season $s$. For example, the $\widehat{\bar{\theta}}_{MLB,2005,\|1\|} = 0.499$ represents the average posterior draw of team strength for 2005 New York Yankees, which ranks first in MLB for that season. Teams ranked first in each season in each sport were matched-up against teams ranked No. 2 - No. 8. To simulate a 'best of $n$' series between the first ranked team (team $i_1$, where $\widehat{\bar{\theta}}_{q,s,|i_1|}$ = $\widehat{\bar{\theta}}_{q,s,|1|}$) and the second ranked team ($i_2$), the following procedure was used in each $q$ and $s$. 

\begin{enumerate}

\item Draw $n$ samples from each the posterior distributions of $\widehat{\theta}_{i_1,q,s}$ and $\widehat{\theta}_{i_2,q,s}$, respectively, labeled as $\widetilde{\theta}_{i_1,q,s}$ amd $\widetilde{\theta}_{i_2,q,s}$, respectively.

\item Draw $n$ samples from the posterior distribution of $\widehat{\alpha_q}$, labeled as $\widetilde{\alpha_q}$. 

\item Estimate $\widetilde{logit}(p_{i_1, i_2, q, s}) = \widetilde{\theta}_{i_1,q,s} - \widetilde{\theta}_{i_2,q,s} + \widetilde{\alpha_q}*\mathds{1*}$, where $\mathds{1*}$ is an $n$-dimensional vector of alternating 1's and -1's to account for rotating home advantages.  

\item Sample from $\widetilde{logit}(p_{i_1, i_2, q, s})$ to obtain 1's and 0's reflecting simulated postseason outcomes. If the sum of these simulations is greater than $n/2$, team $i_1$ wins the series. Otherwise, team $i_2$ wins the series.
