...
\greg{What is table 3 really showing us?  We are combining teams x week x season and then looking at summary statistics of those?  Is that useful?  }  \mike{The most interesting thing IMO is the standard deviation. However, the rest of it seems ancillary, and the standard deviation could be explained in words. Additionally, the standard deviation of all draws (where each team has several thousand) isn't really a great metric now that I think about it}
