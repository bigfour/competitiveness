...
\item Sample one posterior draw of team strength for $\tilde{i}$ and $\tilde{j}$, $\tilde{\theta}_{(q,\tilde{s},\tilde{k}) \tilde{i}}$ and $\tilde{\theta}_{(q,\tilde{s},\tilde{k}) \tilde{j}}$, respectively, from the posterior distributions of $\tilde{i}$ and $\tilde{j}$'s team strength estimates
\item Sample one posterior draw of the HA, $\tilde{\alpha}_{q_0}$, from the posterior distribution of $\alpha_{q_0}$
\item Impute the simulate log-odds of $\tilde{i}$ beating $\tilde{j}$, $\text{logit}(p_{(q,\tilde{s},\tilde{k}) \tilde{i}\tilde{j}}) = \tilde{\theta}_{(q,\tilde{s},\tilde{k}) \tilde{i}} - \tilde{\theta}_{(q,\tilde{s},\tilde{k}) \tilde{j}} +  \tilde{\alpha}_{q_0}$
\item Transform $\text{logit}(\tilde{p}_{(q,\tilde{s},\tilde{k}) \tilde{i}\tilde{j}})$ into probability form to obtain an estimate of $\tilde{p}_{q, sim} = \tilde{p}_{(q,\tilde{s},\tilde{k}) \tilde{i}\tilde{j}}$
\item Repeat the above steps $n_{sim}$ times to obtain $\tilde{p}_{q, 1}, \ldots \tilde{p}_{q, n_{sim}}$.
\end{enumerate}

The steps above were repeated for each $q$ with $n_{sim} = 1000$. Additionally, to remove the effect of each league's HA on simulated probabilities, we also repeated the process using a home advantage of 0 in each league (as if the game were played at a neutral site).
...
\mike{This is still a work in progress. Think the best idea is to tie into area under curve. Highest area = 1 = complete parity, lowest area = 0 = no parity}

Finally, we can quantify the proximity of each league's game-level probabilities to coin flips by comparing the approximate area between each CDF and vertical line at probability 0.5, which reflects the CDF that would be observed if all teams were of equal team strength.  Let $Parity_q$ be our parity measure, such that

$Parity_q$ = $1 - \sum_{sim = 1}^{n_{sim}} \frac{|\tilde{p}_{q, sim} - 0.5|}{n_{sim}}$.

\noindent $Parity_q$ = 1 if all games were coin flips, while $Parity_q$ = 0 if all outcomes were pre-determined. \mike{Tie to AUC?}

For games with no home advantage, $Parity_{MLB} = 0.75$, followed by the NHL (0.72), NFL (0.59), and NBA (0.54). When the best team has a home advantage, parity is again the greatet in the MLB (0.69), followed by the NHL (0.63), NFL (0.47), and NBA (0.39). Taken wholly, this suggests that when the best team is playing at home, the NFL and NBA are closer to a world where every game outcome is pre-determined than one where every game outcome is a coin flip. 
