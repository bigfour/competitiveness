...
Statistical applications in sports have long centered on how to best separate signal, such as team talent, from random noise. However, most of this work has concentrated on a single sport. The development of meaningful cross-sport comparisons has been impeded by the difficulty of translating luck from one sport to another. Building on \cite{glickman1998state}, we present a modified Bayesian state-space approach that provides a uniform understanding of league-level differences in team talent and their evolution.\ben{I didn't understand this last clause, but tried to fix.} Formally, we use game-level probability information provided by betting markets to estimate team strengths across ten seasons of play in each of the National Football League (NFL), National Hockey League (NHL), National Basketball Association (NBA), and Major League Baseball (MLB). This model captures differences in between-season, within-season, and game-to-game variability of team strengths, as well each league's \ben{and each team's?} home advantage. We find that the NBA demonstrates both the largest dispersion in talent and the largest home advantage among the four leagues, while the NHL and MLB stand out for their relative randomness in game outcomes. 
...
\mike{Should this be a conditional expectation?} \ben{it's obviously conditional on the two thetas and alpha, right?}
...
As an example, suppose the Chicago Cubs were favored ($-127$ on the money line) to beat the Arizona Diamondbacks ($117$). Mathematically, if the line is $\ell$, then the implied probability $p(\ell)$ of a win is given by:
$$
  p(\ell) = \begin{cases}
...
While we are unable to share our betting market data due to licensing restrictions, the data wrangling code, Gibbs sampling code, posterior draws, \st{game results}, and the code used to obtain posterior estimates and figures are all posted to a GitHub repository, available at \url{https://github.com/bigfour/competitiveness}.
...
\ben{Maybe so, but I usually like having the table as well, since you can see the actual numbers.}
...
Figure \ref{fig:alphaAll} depicts substantial between-franchise differences within both the NBA and NHL. 
\mike{Should we make this plot grey and black? I'm not convinced the colors add anything, particularly for a journal article. We can also do the boxplots/dotplots that were proposed over email} 
\ben{I agree. The colors are cool but I'm not sure they add anything for JASA.}
Conversely, HA estimates within the NFL and MLB are, by and large, more consistent across franchises. Interestingly, the home advantages of a few NFL franchises are skewed (see Denver and Seattle, relative to Detroit), potentially the result of a shorter regular season. Alternatively, HA may vary by season, game time, or the day of the game. Anecdotally, night games (Thursday, Sunday, or Monday) conceivably offer a larger HA than those played during the day \citep{Seahawks}. Informally, team-level HA estimates are similar in effect size to those depicted by \cite{koopmeiners2012comparison} in the NFL.
...
\subsection{How often does the best team win?}

One of our initial questions regarded the inherent randomness of game outcomes. While sportsbook odds can approximate the probability of each team winning, these odds are only provided for scheduled games. As a result, any inferences would be contingent upon how each league schedules its games. 

One approach that can better assess the equivalence of teams in each league would consider the likelihood that, given two teams chosen at random, the better team won. If the better team in a randomly chosen match-up wins more often, it would correspond to a sport with less inherent randomness. Using our posterior draws, we approximate the distribution of game-level probabilities between two randomly chosen teams using the following steps.
...
\item Draw week $\tilde{k}$ from $\left{1, \ldots , s^{\star}_{q}\right\}$
\item Draw teams $\tilde{i}$ and $\tilde{j}$ from $\left{1, \ldots , t_{q}\right\}$ without replacement. Assign $\tilde{i}$ to be the team with the better average posterior distribution of team strength during season $\tilde{s}$ and week $\tilde{k}$
\item Sample one posterior draw of team strength for $\tilde{i}$ and $\tilde{j}$, $\tilde{\theta}_{(q,\tilde{s},\tilde{s}) \tilde{i}}$ and $\tilde{\theta}_{(q,\tilde{s},\tilde{s}) \tilde{j}}$, respectively, from the posterior distributions of $\tilde{i}$ and $\tilde{j}$'s team strength estimates
\item Sample one posterior draw of the HA, $\tilde{\alpha}_{q_0}$, from the posterior distribution of $\alpha_{q_0}$
\item Impute the simulate log-odds of $\tilde{i}$ beating $\tilde{j}$, $\text{logit}(p_{(q,\tilde{s},\tilde{k}) \tilde{i}\tilde{j}}) = \tilde{\theta}_{(q,\tilde{s},\tilde{s}) \tilde{i}} - \tilde{\theta}_{(q,\tilde{s},\tilde{s}) \tilde{j}} +  \tilde{\alpha}_{q_0}$
\item Transform $\text{logit}(\tilde{p}_{(q,\tilde{s},\tilde{k}) \tilde{i}\tilde{j}})$ into probability form to obtain an estimate of $\tilde{p}_{(q,\tilde{s},\tilde{k}) \tilde{i}\tilde{j}}$
\item Repeat the above steps 1000 times
\end{enumerate}

The steps above were repeated for each $q$. Additionally, to remove the effect of each league's HA on simulated probabilities, we also repeated the process using a home advantage of 0 in each league (as if the game were played at a neutral site).

Figure \ref{BestWin} shows the cumulative distribution functions for each set of probabilities. 
...
For more than 90\% of MLB and NHL games played at neutral sites, match-ups are less than a 2:1 proposition with respect to the best team winning (66\%). Meanwhile, more roughly 50\% of NFL and NBA match-ups are greated than this threshold. 

The importance of each league's home advantage also makes a meaningful difference. When the best team plays at home in the NBA, that squad is almost always favored to win 60\% of the time, with the middle 50\% of games ranging from a 70\% probability to an 84\% probability. Meanwhile, even with a home advantage, it is rare that the best MLB team is $ever$ given a 70\% probability of winning, with the middle 50\% of games ranging from 56\% to 63\%. 

Finally, we can quantify the proximity of each league's game-level probabilities to coin flips by comparing the approximate area between each CDF and a vertical line at probability 0.5, which reflects the CDF that would be observed if all teams were of equal team strength. For games with no home advantage, the MLB's area to the left of the CDF (ATLC) is the smallest at 25\%, followed by the NHL (28\%), NFL (41\%), and NBA (46\%). When the best team has a home advantage, the ATLC is again smallest in MLB at 31\%, followed by the NHL (37\%), NFL (53\%), and NBA (61\%). Taken wholly, this suggests that when the best team is playing at home, the NFL and NBA are closer to a world where every game outcome is pre-determined than one where every game outcome is a coin flip. 

%The areas to the left of each CDF are also interesting are about 8% (MLB), 12% (NHL), 27% (NFL), and 34% (NBA), done without any home advantage.
 
%The areas to the left of each CDF are about 14% (MLB), 21% (NHL), 40% (NFL), and 53% (NBA), done with a constant league-level home advantage.
 
%In other words, if you give the best team in each possible match-up a home advantage, the NBA is closer to a world where every game-winner is predetermined than where every game is a coin flip.
