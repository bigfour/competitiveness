...
The joint distribution of the variability in team strength on a season-to-season ($\tau_{q,season}$) and week-to-week ($\tau_{q,week}$) basis is shown in via contour plots in Figure~\ref{fig:contourSigma}, using separate colors for each $q$. 
...
<<alphaAll, fig.height = 10.8, fig.width = 8, fig.cap="Median posterior draw (with 2.5th, 97.5th quantiles) of each franchise's home advantage intercept, on the log odds scale">>=
gg2 
@
