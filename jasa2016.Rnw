...
\mike{This is quite similar to what's above - we should pick one and get rid of the other one.}
In circumstances where more than one betting line was available for a particular game, we included only the line from before the game started (this is known as the closing line). These betting lines are expressed as payouts, which we subsequently convert into implied probabilities. \mike{Change because this is not the first game anymore} For example, in the first game in our data set, the Chicago Cubs were favored ($-127$) to beat the Arizona Diamondbacks ($117$) on April 4th, 2005, despite being on the road. Mathematically, if the line is $\ell$, then the implied probability $p(\ell)$ of a win is given by:
$$
  p(\ell) = \begin{cases}
