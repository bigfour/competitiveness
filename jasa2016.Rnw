...
One immediate hurdle in building a cross-sport model is that in addition to having different numbers of teams in each league, certain franchises may relocate from one city to another over time. In our data, there were two relocations, Seattle to Oklahoma City (NBA, 2008) and Atlanta to Winnipeg (NHL, 2011). Let $\alpha_{q_{0}}$ be the league-wide home advantage (HA) in $q$, and let $\alpha_{(q) i^{\star}}$ be the extra effect (positive or negative) on team $i$ for game's played in venue $i^{\star}$, for $i^{\star} = 1, \cdots t^{\star}_{q}$. Here, $t^{\star}_{q}$ is the total number of home venues; in our data, $t^{\star}_{MLB} = 30$, $t^{\star}_{NBA} = t^{\star}_{NHL} = 31$, and $t^{\star}_{NFL} = 32$.

\mike{Just realized that our $\alpha_{(q) i}$'s aren't the home advantage for team $i$; instead, they are more like team level random effects, adding or subtracting from the league average. I called it a 'home effect'}
...
$$
\greg{I think we need to add $\alpha_{q_{0}}$ to $\alpha_{(q)i}$} \mike{Agree. Added in.} \\
...
\mike{Nightmare scenario: A reviewer asks us to find all the neutral site games and correctly identify them as such}
...
\mike{Question about notation of $\mathbf{\theta_{q,s,g_{q,s,.}}}$. Why isn't this just $\mathbf{\theta_{q,s,k^{\star}_{q}}}$} \greg{There used to be different a potentially different number of weeks in different serasons within the same sport.  But you fixed that at some point, (I think) so they all seasons within a sport have the same number of weeks.  So we can accept your suggested change.  }\greg{g is the number of games.  It shouls refer to the dimension of some matrices.  But it's not an index.  I'll fix this.  }

and 
\greg{Does going from one season to the next invoke both the week gamma and the season gamma?  Or just the season gamma?  If the latter, which I think is what we did, then we may need to add a statement like ``for $k+1 \< k^{\star}_q$}
$$
...
$$
\greg{I think we need different indices.  These both shouldn't be $i$.  Maybe $i$ and $i'$?}
...
<<<<<<< HEAD
Taken wholly, this suggests that perceived differences in the home advantage between teams in each league are likely not accounted for due to chance alone, with NBA teams showing the largest team-to-team differences in the HA. As such, results that follow use model estimates from Model (\ref{model.full}). \greg{It's equation (3) but I think it's confusing to call this model 3.  Can we think of a better way to label these models?  }
=======
Taken wholly, this suggests that perceived differences in the home advantage between teams in each league are likely not accounted for due to chance alone, with NBA teams showing the largest team-to-team differences. As such, results that follow use model estimates from Model (\ref{model.full}). 
>>>>>>> 55f07233e2d9dca1232f7c17feb5143706c8bbcc

\subsection{Team strength}


\greg{What is table 3 really showing us?  We are combining teams x week x season and then looking at summary statistics of those?  Is that useful?  }
...
<<spaghetti, fig.height=13.5, fig.cap="Team strengths over time for all four sports leagues. MLB and NFL seasons follow each yearly tick mark on the x-axis, while NBA and NHL seasons occur during years labeled by adjacent tick marks.">>=
...
<<<<<<< HEAD
As a final point of clarification in Figures \ref{fig:spaghetti}, \ref{fig:spaghetti-nba}, and \ref{fig:spaghetti-nhl}, the periods of time with straight lines of team strength estimates in the 2012-13 season (NHL) and 2011-12 season (NBA) reflect games lost due to lockouts. 
=======
As a final point of clarification in Figures \ref{fig:spaghetti}, \ref{fig:spaghetti-nba}, and \ref{fig:spaghetti-nhl}, the periods with straight lines of team strength estimates in the 2011-12 season (NBA) and 2012-13 season (NHL) reflect games lost due to lockouts. 
>>>>>>> 55f07233e2d9dca1232f7c17feb5143706c8bbcc
...
\greg{Agreed.  Standard deviation is more interesting.  }
...
<<<<<<< HEAD
Figure \ref{fig:alphaAll} shows the 2.5th percentile, median, and 97.5th percentile draws of each team's estimated home advantage parameter, presented on the log-odds scale. These are calculated by summing draws of $\alpha_{q_0}$ with $\alpha_{(q)i}$ for all $i$. \greg{This is why we need a different index for teams and arenas.} HAs are shown in order to provide a sense of the magnitude of differences between the home advantage provided in the MLB (increased log-odds of a home win around 0.16, or about $e^{0.16} - 1 = 17$\% higher), NHL (0.22, or 25\%), NFL (0.36, or 43\%), and NBA (0.50, or 62\%).  The two franchises that have relocated in the last decade, the Atlanta Thrashers (NHL) and Seattle Supersonics (NBA), are also included for the games played in those respective cities. 
=======
Figure \ref{fig:alphaAll} shows the 2.5th, median, and 97.5th draws of each team's estimated home advantage parameter, presented on the log-odds scale. These are calculated by summing draws of $\alpha_{q_0}$ with $\alpha_{(q) i^{\star}}$ for all $i^{\star}$ and for all $q$. HAs are shown in order to provide a sense of the magnitude of differences between the home advantage provided in the MLB (increased log-odds of a home win around 0.16, or about $e^{0.16} - 1 = 17$\% higher), NHL (0.22, or 25\%), NFL (0.36, or 43\%), and NBA (0.50, or 62\%).  The two franchises that have relocated in the last decade, the Atlanta Thrashers (NHL) and Seattle Supersonics (NBA), are also included for the games played in those respective cities. 
>>>>>>> 55f07233e2d9dca1232f7c17feb5143706c8bbcc
...
\greg{Our models don't, but our data does.....}
...
\greg{Figure 6, is missing the solid black line.  }
...
\greg{I don't understand step six.  50\% prob of positive or negative? }
...
<<<<<<< HEAD
The steps above were repeated for each $q$ with $n_{sim} = 1000$. Additionally, to remove the effect of each league's HA on simulated probabilities, we repeated the process used by fixing $\tilde{\alpha}_{q_0} = 0$ for each league, reflecting the probability that would have been given for the game being played at a neutral site.
=======
