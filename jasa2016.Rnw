...
\citep{glickman1998state} and \citep{glickman2016estimating}) built on the BTM by allowing team-strength estimates to vary over time using a state-space model in the NFL.  Let $y_{ij}$ be the outcome of the game between $i$ and $j$, where $i$ and $j$ take on values between $1$ and $t$ where $t$ is the number of teams in the league (at the time, $t=28$).  Game outcomes are assuming to follow approximately a normal distribution, and the parameter $\theta_{(s,k)i}$ is the strength of team $i$ in season $s$ during week $k$.  \citep{glickman1998state} incorporate an individual home field advantage parameter for the $i$-th team, $\alpha_i$, where again $\alpha$ ranges in value from $1$ to $t$.  Using this notation the expected outcome margin of a game between team $i$ and team $j$ played in stadium $i$ during week $k$ in season $s$ is: 
$$
...
$$
\theta_{(s+1,1)} | \gamma_{seas}, \bf{\theta_{s,g_{s}}}, \tau^{2}_{seas},  \sim N (\gamma_{seas}\bf{G}\bf{\theta}_{(s,g_{s})},(\tau^{2}_{seas})I_{t})
$$

$$
\theta_{(s,k+1)} | \gamma_{week}, \bf{\theta_{s,k}}, \tau^{2}_{week},  \sim N (\gamma_{seas}\bf{G}\bf{\theta}_{(s,k)},(\tau^{2}_{week})I_{t})
$$

We depart from \citep{glickman1998state} here in our specification of precision.  Here, we estimate three separate parameters, $\tau^{2}_{error}$, $\tau^{2}_{season}$, and $\tau^{2}_{week}$.  This allows these three parameters to be estimated separately.  
...
bigfour %>%
  ...
  mutate(coverage = N_bets / N) %>%
  xtable(caption = "Summary of cross-sport data. Note that we have perfect coverage (betting odds for every game) across all four major sports during the 2005--2014 regular seasons.", 
...
markets_plot <- ggplot(data = bigfour, 
                       aes(x = p_home, y = as.numeric(home_win), color = sport)) + 
  geom_point(alpha = 0.1) + 
  geom_point(data = bigfour_binned, 
             aes(x = p_home_bin, y = home_win_bin_pct, size = N), alpha = 0.5) + 
  geom_smooth() +
  geom_abline(slope = 1, intercept = 0, color = "black", lty = 2) + 
  geom_hline(data = bigfour_summary, aes(yintercept = home_win_pct, color = sport), lty = 3) + 
  geom_vline(data = bigfour_summary, aes(xintercept = home_win_prob, color = sport), lty = 3) + 
  coord_equal() + 
...
<<>>=
load(file.path(mcmc_dir, "/nba_7_23_constantHFA.RData"))

library(ggplot2)
library(gplots)
library(reshape2) # for melt
test<-hist2d(1/z$sigmab[,,1],1/z$sigmabSeason[,,1],nbins=20)

greg<-melt(test$counts)
names(greg) <- c("x", "y", "z")

temp<-substring(as.character(greg[,1]),2,nchar(as.character(greg[,1]))-1) 
temp<-do.call(rbind,strsplit(temp,","))
temp<-apply(temp,2,function(x){as.numeric(x)})
temp<-apply(temp,1,mean)
greg$x<-temp

temp<-substring(as.character(greg[,2]),2,nchar(as.character(greg[,2]))-1) 
temp<-do.call(rbind,strsplit(temp,","))
temp<-apply(temp,2,function(x){as.numeric(x)})
temp<-apply(temp,1,mean)
greg$y<-temp

greg$z<-greg$z/sum(greg$z)
gregNBA<-greg

load(file.path(mcmc_dir, "/nhl_7_23_constantHFA.RData"))
...
load(file.path(mcmc_dir, "/mlb_7_23_constantHFA.RData"))
...
<<contour, fig.cap="Greg's insane contour plot.">>=
v <- ggplot(greg, aes(x, y, z = z))
v + geom_contour(bins=8) + labs(x=expression(sigma[w]^2),y=expression(sigma[s]^2),title="NFL")
dev.off()


v <- ggplot()
v + 
  geom_contour(data=gregNFL, aes(x, y, z = z),bins=8,col="blue") + 
  geom_contour(data=gregNBA, aes(x, y, z = z),bins=8,col="red") + 
  geom_contour(data=gregMLB, aes(x, y, z = z),bins=3,col="purple") + 
  geom_contour(data=gregNHL, aes(x, y, z = z),bins=5,col="green") + 
  labs(x=expression(sigma[w]^2),y=expression(sigma[s]^2),title="SPORTS")  
