...
\newcommand{\mike}[1]{\sethlcolor{pink}\hl{[SJ]: #1}}
...
\title{Cross-Sport Competitiveness}
%
\author{
Benjamin S. Baumer \\
...
The importance of quantifying team strength in sport has extended across disciplines. This includes work done in economics on contrasting league-level characteristics \citep{leeds2004economics}, in statistics on estimating game-level probabilities \citep{glickman1998state}, and in forecasting on classifying future game winners \cite{boulier2003predicting}. We highlight and attempt to coalesce general approaches below. 
...
Let $p_{(s,k)ij}$ correspond to the probability that team $i$ will beat team $j$ in season $s$ during week $k$.  These quantities are estimated to give $\hat{p}_{(s,k)ij}$ using the implied odds of the betting market given the money line odds for a game where team $i$ plays team $j$ in week $k$ of season $s$.  

We then fit the model

$$
logit(p_{(s,k)ij}) \sim N(\bf{\theta_{(s,k)}}\bf{X}_{s,k} + \bf{\alpha}\bf{Z}_{s,k}, \tau_{error}\bf{I}_{g_{(s,k)}})
$$

where $\bf{\theta_{(s,k)}}$ is a vector of length $t$ containing the team strength parameters in season $s$ during week $k$ and $\bf{\alpha}$ is a vector of length $t$ containing home field advantage parameters that do not vary over time (i.e. HFA is assumed to be constant for a team over weeks and seasons). $\bf{X}_{s,k}$ and $\bf{Z}_{s,k}$ are both of dimension $g_{(s,k)}$ (i.e. the number of games in season $s$ of week $k$.) by $t$.   The matrix $\bf{X}_{s,k}$ contains the values 1, 0, and -1 where for a given row (i.e. one game) the value of $i$-th column in that row is a 1/-1 if the $i$-th team played at home/away in the given game and 0 otherwise.  $\bf{Z}_{s,k}$ is a matrix containing a 1 in the $i$-th column if the $i$-th team played the game corresponding to that row at home and 0 otherwise.  Finally, $\tau_{error} =  \frac{1}{\sigma_{error}}$ is the precision and $\bf{I}_{n_{(s,k)}}$ is an identity matrix with  dimension ${n_{(s,k)}}$ by ${n_{(s,k)}}$.  

Similarly to \cite{glickman1998state}, we allow the strength parameters of the teams to vary from week to week and from season to season.  

$$
\theta_{(s+1,1)} | \gamma_{seas}, \bf{\theta_{s,g_{s}}}, \tau_{seas},  \sim N (\gamma_{seas}\bf{G}\bf{\theta}_{(s,g_{s})},(\tau_{seas})I_{t})
$$

$$
\theta_{(s,k+1)} | \gamma_{week}, \bf{\theta_{s,k}}, \tau_{week},  \sim N (\gamma_{seas}\bf{G}\bf{\theta}_{(s,k)},(\tau_{week})I_{t})
$$

We depart from \cite{glickman1998state} here in our specification of precision.  Here, we estimate three separate parameters, $\tau_{error}$, $\tau_{season}$, and $\tau_{week}$.  This allows these three parameters to be estimated separately.  

PUT PRIORS FOR EVERYTHING HERE.
$$
\theta \sim 
\alpha \sim
\gamma_{week}
...
Points from \cite{glickman2016estimating}: 
...
We borrowed from \cite{glickman1998state}.
