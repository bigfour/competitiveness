...
One of our initial questions regarded the inherent randomness of game outcomes. While sportsbook odds can approximate the probability of each team winning, these odds are only provided for scheduled games. As a result, any between-league comparisons would be contingent upon each league's schedule. 

To better assess the equivalence of all teams teams in each league, we consider the likelihood that, given any pair of teams chosen at random, the better team won. If the better team in a randomly chosen match-up wins more often, it would correspond to a sport with less inherent randomness. Using our posterior draws, we approximate the distribution of game-level probabilities between two randomly chosen teams using the following steps.
...
\item Impute the simulate log-odds of the best team winning between $\tilde{i}$ beating $\tilde{j}$, $\text{logit}(p_{(q,\tilde{s},\tilde{k}) \tilde{i}\tilde{j}}) = \tilde{\alpha}_{q_0} + \|\tilde{\theta}_{(q,\tilde{s},\tilde{k}) \tilde{i}} - \tilde{\theta}_{(q,\tilde{s},\tilde{k}) \tilde{j}} + \tilde{\tau}_{q, game}\|$, where the best team winning is based on $\tilde{\theta}_{(q,\tilde{s},\tilde{k}) \tilde{i}}$, $\tilde{\theta}_{(q,\tilde{s},\tilde{k}) \tilde{j}}$, and $\tilde{\tau}_{q, game}$. This is meant to best approximate draws of team strengths and sport-specific error for any given game. The home advantage is always assigned to team considered to be better for any given contest.
...
The steps above were repeated for each $q$ with $n_{sim} = 1000$. Additionally, to remove the effect of each league's HA on simulated probabilities, we repeated the process using by fixing $\tilde{\alpha}_{q_0} = 0$ for each league (as if the game were played at a neutral site). 

Figure \ref{fig:BestWin} shows the cumulative distribution functions (CDF) for each set of probabilities in each league. 
...
Nearly every MLB and NHL games played at neutral sites is less than a 3:1 proposition with respect to the best team winning (75\%). Meanwhile, more roughly 30\% of NBA and 23\% of NFL neutral site match-ups are greated than this threshold. 

The importance of each league's home advantage also makes a meaningful difference. When the best team plays at home in the NBA, that squad is always favored to win at least 60\% of the time, with the middle 50\% of games ranging from a 70\% probability to an 85\% probability. Meanwhile, even with a home advantage, it is rare that the best MLB team is $ever$ given a 70\% probability of winning, with the middle 50\% of games ranging from 57\% to 63\%. 
...
Finally, we use the CDF curves displayed above to quantify the proximity of each league's game-level probabilities to coin flips by estimating the approximate area under each CDF. Let $Parity_q$ be our parity measure, such that

$Parity_q = 2*\int_{0.5}^1 P(\tilde{p}_{q, sim} \leq x) dx$. We multiply by 2 to scale so that $0 \leq Parity_q \leq 1$, where 1 represents complete parity (every game a coin flip) and 0 represents no parity (every game outcome pre-determined) \mike{Tie to AUC?}

For games with no home advantage, $Parity_{MLB} = 86\%$, followed by the NHL (83\%), NFL (68\%), and NBA (64\%). When the best team has a home advantage, parity is again the greatet in the MLB (79\%), followed by the NHL (73\%), NFL (54\%), and NBA (46\%). Taken wholly, this suggests that when the best team is playing at home, the NBA is closer to a world where every game outcome is pre-determined than to one where every game outcome is a coin flip. Meanwhile, even when giving the best team a HA, baseball game outcomes remain lightly-weighted coin flips. 
