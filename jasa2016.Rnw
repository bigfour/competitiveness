...
Figure XX-XY show estimated team coefficients over time, approximated using the posterior mean draws from Model (\ref{model.full}). Each team in Figures XX-XY are depicted using their two primary colors, taken from \url{http://jim-nielsen.com/teamcolors/} via the `teamcolors` package (\url{https://github.com/beanumber/teamcolors}) in R.
