...
One immediate hurdle in building a cross-sport model is that in addition to having different numbers of teams in each league, certain teams will relocate from one city to another over time. In our data, there were two franchise relocations, Seattle to Oklahoma City (NBA, 2008) and Atlanta to Winnipeg (NHL, 2011). $\alpha_{(q) i^{\star}}$ is the home advantage (HA) parameter for a game played in the current home venue of franchise $i$, such that $i^{\star} = 1, \cdots t^{\star}_{q}$, where $t^{\star}_{q}$ is the total number of home venues in our data. Note that $t_{q} \le t^{\star}_{q}$ due to franchise relocation. 



Letting $\theta_{(q,s,k) i}$ and $\theta_{(q, s, k) j}$ be season-week team strength parameters for teams $i$ and $j$, respectively, where $i,j = 1, \cdots, t_{q}$, we assume that 
...
\greg{I think we need to say after the model $i,j = 1, \cdots, t_{q}$, $q = 1,\cdots, 4$, etc.} \mike{We have the space for $q$ in the first sentence, and I added the space for $i$ and $j$ above. Does that work? I also added the space for $i^{\star}$ below.}\\
