...
Let $p_{(q,s,k)ij}$ be the probability that team $i$ will beat team $j$ in season $s$ during week $k$ of sports league $q$, where $q \in \{MLB, NBA, NFL, NHL\}$. These probabilities are assumed to be known, calculated using $p(\ell)$ via sportsbook odds for teams $i$ and $j$. Letting $\theta_{(q,s,k) i}$ and $\theta_{(q, s, k) j}$ be season-week team strength parameters for teams $i$ and $j$, respectively, we assume that 

$$
E[logit(p_{(q,s,k) ij})] = \theta_{(q,s,k) i} - \theta_{(q, s, k) j} + \alpha_{(q) i},
$$
\mike{I added the corresponding fit from the constant HA model}
...
\noindent For Model (\ref{model.red}), matrices $\mathbf{X}_{q,s,k}$, $\mathbf{J}_{g_{q,s,k}}$, and $\mathbf{I}_{g_{(q,s,k)}}$ are specified identically to Model (\ref{model.full}).
...
tidy_betas %>%
  ...
  xtable(caption = "Summary of team strength parameters, taken on the log-odds scale. N*: number of unique team strength draws (teams x seasons x weeks)", 
...
The joint distribution of the variability in team strength on a season-to-season ($\tau_{q,season}$) and week-to-week ($\tau_{q,week}$) basis is shown in contour plots in Figure~\ref{fig:contourSigma}, using separate colors for each $q$. 
