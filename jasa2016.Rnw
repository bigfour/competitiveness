...
\subsection{Posterior estimates & competitive balance}

Figure XX-XY show estimated team coefficients over time, approximated using the posterior mean draws from Model (\ref{model.full}). Each team in Figures XX-XY are depicted using their two primary colors, taken from \url{http://jim-nielsen.com/teamcolors/} into R via the `teamcolors` package (\url{https://github.com/beanumber/teamcolors}).

Team equivalence at a single snapshot in time can be estimated by taking the standard deviation of team strength estimates shown in Figures XX-XY, while season-to-season and week-to-week variability are approximated by posterior draws of \gamma_{q,seas} ($p(\tilde{x})) and \gamma_{q,week}, respectively. 
