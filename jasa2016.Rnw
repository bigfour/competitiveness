...
As an example, suppose the Chicago Cubs were favored ($-127$ on the money line) to beat the Arizona Diamondbacks ($117$). Mathematically, if a team's line is $\ell$, then the implied probability of a win for that team, $p(\ell)$, is given by:
$$
  p(\ell) = \begin{cases}
