...
\newcommand{\blind}{\author{Three anonymous authors}}
%\newcommand{\Smith}{XXX\xspace}
%\newcommand{\Smathies}{XXXies\xspace}
...
\noindent In Model CFA, matrices ${\bf p}_{(q,s,k)}$, $\mathbf{X}_{(q,s,k)}$, $\mathbf{J}_{g_{(q,s,k)}}$, and $\mathbf{I}_{g_{(q,s,k)}}$ are specified identically to Model IFA. As a result, for a game between home team $i$ and away team $j$ during week $k$ of season $s$, $E[\text{logit}(p_{(q,s,k) ij})] = \theta_{(q,s,k) i} - \theta_{(q, s, k) j} + \alpha_{q_0}$ under Model CFA. 
...
@
\mike{Forgot from phone call: We want to keep Q1 and Q3, right?}\ben{I think so. One thing -- in the text above we reference that the NBA and NFL numbers are typically between -1 and 1. That was based on the narrow interval before. Should we change it to -1.2 to 1.2? Or restrict to the NFL and forget about the NBA?}
...
levels(cdf.all$sport) <- sort(levels(cdf.all$sport))
gg5 <- ggplot(cdf.all) + 
