...
The joint posterior distribution of $\gamma_{q,seas}$ and $\gamma_{q,week}$ is shown in Figure YY, which uses contour plots with separate colors for each sport. For each of the NHL, NBA, and NFL, the posterior estimate of $ \gamma_{q,week}$
