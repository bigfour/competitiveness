...
\noindent Note that we cap $\gamma_{q,week}$ and $\gamma_{q,season}$ at 1.5 and 1.0, respectively, corresponding to prior beliefs in whether or not team strengths could explode within (unlikely, but feasible) or between (highly unlikely) seasons. 
...
tidy_alpha_prob <- tidy_alphas %>%
  mutate(alpha_prob = i.arcsin(alpha.team.overall), ## alpha_prob = mosaic::ilogit(alpha.team.overall)
...
gg4 <- ggplot(data = tidy_alpha_prob, 
  ...
  geom_errorbar(aes(ymin = i.arcsin(alpha.team.lower), 
                    ymax = i.arcsin(alpha.team.upper))) +
  geom_point() + 
  coord_flip() + 
