...
\hl{Our secondary interest liesin the predictive capability of our team strength estimates with respect to the information sharing of state-space frameworks. Models IHA and CHA each blend outcomes from weeks prior to $k$, during $k$, and after $k$ to estimate team strength. While this is ideal for measuring league parity, it does not work so well with game predictions. As such, in each $q$ for season $S_q$ (the last season of our data), we also fit a series of state-space models using Model IHA, done on a weekly basis (these are termed $sequential$ fits, as opposed to $cumulative$. Formally, for $k = 2, \ldots, K_{q}$ in season $S_q$, we use Model IHA only on games during $k$ or prior.} \hl{Note: see how Glickman worded his prediction section}
...
<<R2, fig.height = 3, fig.width = 6, fig.cap="Coefficient of determination with future in-season win percentage. We note the improvement our team strength estimates offer over season-to-date win percentage and season-to-date point differential in all sports, especially early in the season. ">>=
gg.r2
@
