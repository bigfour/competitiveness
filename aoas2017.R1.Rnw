...
\begin{abstract}
Statistical applications in sports have long centered on how to best separate signal, such as team talent, from random noise. However, most of this work has concentrated on a single sport, and the development of meaningful cross-sport comparisons has been impeded by the difficulty of translating luck from one sport to another. In this manuscript, we develop Bayesian state-space models using betting market data that can be uniformly applied across sporting leagues to better understand the role of randomness in game outcomes. These models can be used to extract estimates of team strength, the between-season, within-season, and game-to-game variability of team strengths, as well each team's home advantage. \hl{More generally, we use our framework to compare predictions from cumulative (across all weeks) and sequential (from all weeks prior) models, finding that sequential models, although boasting less accurate predictions than cumulative models, are still able to capture the majority of team-level information that changes across time.} \greg{I don't like the idea of using the term ``predictions" with the cumulative model.} We implement our approach across a decade of play in each of the four major North American sports leagues: National Football League (NFL), National Hockey League (NHL), National Basketball Association (NBA), and Major League Baseball (MLB).  We find that the NBA demonstrates both the largest dispersion in talent and the largest home advantage, while the NHL and MLB stand out for their relative randomness in game outcomes. We conclude by proposing a new metric for judging league competitiveness that works in absence of factors outside of team control \greg{What do we mean by this: ``Works in the absence of factors outside of team control"?}. 
\end{abstract}
...
However, because randomness manifests differently in different sports, a limitation of sport-specific models is that inferences cannot generally be applied to other competitions. As a result, researchers who hope to contrast one league to another often focus on the one outcome common to all sports: won-loss ratio \greg{Should we say winning percentage here?}. Among other flaws, measuring team strength using wins and losses performs poorly in a small sample size, ignores the game's final score (which is known to be more predictive of future performance than won-loss ratio \citep{boulier2003predicting}), and is unduly impacted by, among other sources, fluctuations in league scheduling, season length, injury to key players, and the general advantage of playing at home. As a result, until now, analysts and fans have never quite been able to quantify inherent differences between sports \greg{should we say leagues instead of sports? What we are doing, for instance, doesn't apply to basketball in general, but specifically, the NBA} with respect to randomness and the dispersion and evolution of team strength. We aim to fill this void. 

In the sections that follow, we present a novel approach for estimating team ability that accounts for league scheduling, measures each team's home advantage, and uncovers inherent differences in North American sports leagues. First, we validate an assumption that game-level probabilities provided by betting markets provide \greg{approximately?} unbiased and low-variance estimates of the true probabilities of wins and losses in each professional contest. Second, we implement \greg{modified from what?} \hl{modified Bayesian state-space models} that use these implied probabilities to capture estimates of team strength and variability, finding that our estimates of team strength improve upon both won-loss ratio and point differential with respect to correlating to future performance. \hl{Given that state-space models use information from future time points to inform estimates at current time points, we compare our cumulative fits to sequential models, fit iteratively over time within each sport.} Next, we use posterior estimates of within and between season variability \greg{Are you talking about the sigmas or the gammas?} to present unique league-level contrasts \greg{properties?} that to this point have been difficult to capture. Finally, we propose a novel metric for assessing league parity using the posterior distributions of team strength parameters. We find that, on account of both narrower talent distributions \greg{narrower team strength distribution?  Talent distribution sounds like we are talking about players.} and smaller home advantages, a typical contest in the NHL or MLB is much closer to a coin-flip than one in the NBA or NFL. 
...
\hl{Our secondary interest lies in using estimated team strengths to better gauge the equivalence of each league's teams; i.e., how likely was it or will it be for each team to beat other teams? In this respect, we are interested in both looking backwards across time (descriptive) as well as looking forwards (predictive). However, Models IHA and CHA each blend outcomes from weeks prior to $k$, during $k$, and after $k$ to estimate team strength. While this is ideal for measuring league parity looking backwards, it is less appropriate to make future game predictions. As such, in each $q$ for season $S_q$ (the last season of our data), we fit a series of state-space models using Model IHA, done on a weekly basis (these are termed $sequential$ fits, as opposed to $cumulative$). Formally, for $k = 2, \ldots, K_{q}$ in season $S_q$, we also fit Model IHA only on games during $k$ or prior. The comparison of $sequential$ versus $cumulative$ fits of team strength can help us gauge how much state-space model estimates are due to future outcomes, as well as providing a sense of the predictive capability of our estimates.}
...
Ultimately, estimates from Model IHA are designed to estimate team quality at any given point in a season in the absence of factors such as the home advantage and opponent caliber. If these estimates more properly assess team quality than other metrics of team success (e.g., won-loss percentage or point differential), they should more accurately link to future performance, such as how well teams will perform over the remainder of the season. \hl{Additionally, game-level probabilities estimated from IHA coefficients should closely link to observed money lines.}

\hl{However, it's admittedly unfair to use $cumulative$ estimates of team strength, as future game outcome information is implicity used to predict those same game outcomes. In this sense, $sequential$ fits may more appropriate for understanding the predictive capability of our state-space models. In this section, we present two ways of comparing the predictive accuracy of both $sequential$ and $cumulative$ state-space fits.}

First, Figure ~\ref{fig:R2} shows the coefficient of determination ($R^2$) between each team's future won-loss percentage in a season and each team's (i) average team strength estimate from $cumulative$ Model IHA, \hl{(ii) average team strength estimate from $sequential$ Model IHA}, (iii) season-to-date cumulative point differential, and (iv) season-to-date won-loss percentage. Within each sport, this is computed by game number, which helps to account for league-level differences in season length. \hl{For purposes of using our $cumulative$ team strength estimates, we took the mean posterior draw for each team in each week a particular contest was played; for $sequential$, we used the mean posterior draw for each team in the week prior.}
...
Across each sport, our estimates of team strength consistently outperform past team win percentage and point differential in predicting future win percentage. This gap is most pronounced earlier in each season, which is not surprising given the instability of won-loss percentage and point differential in a small number of games. \hl{In general, our $cumulative$ estimates better correlate to future performance than our $sequential ones$. Differences in predictive accuracy remain throughout most of the regular season in MLB, the NHL, and the NFL.} However, by the NBA's mid-season, won-loss ratio and point differential are similar to our estimates of team strength in assessing future performance. By and large, this confirms the findings of \cite{wolfson2015s}, who identified that most of the information needed to predict the remainder of the NBA season is contained within the first third of the year.

\hl{As a secondary comparison of $cumulative$ and $sequential$ state-space fits, we use the average posterior draws of team strength and franchise-specific home advantages at each week to extract game-level probabilities. For $sequential$ IHA fits, probabilities are estimated using team strengths from fits ending the week prior, to ensure that no future outcomes are used to inform predictions. Table} \ref{tab:ll} highlights the average and median log-loss in each sport comparing observed probabilities (our $p_{(q,s,k)ij}$'s) versus estimated $p_{(q,s,k)ij}$'s imputed using both $cumulative$ and $sequential$ fits. \hl{Log-loss is a proper scoring rule for probabilistic predictions} \citep{gneiting2007strictly} and is a commonly used approach for assessing model accuracy in sports \citep{lopez2015building, ruiz2015generative, buttrey2016beating}.
...
#ggplot(bigfour.last, aes(p_homeS, p_homeC)) + geom_point() + geom_smooth() + geom_abline(intercept = 0, slope = 1) + facet_wrap(~sport)
...
p1 <- ggplot(temp, aes(x = logloss, colour = type, fill = type)) + geom_density(alpha = 0.2) + 
   ggtitle("Density curves of game level log loss") + 
   geom_vline(xintercept = log(0.5), lty = 2) + xlim(-2.2, 0) + 
   annotate("text", x = -1, y = 1.2, label = "Coin flip")  + facet_wrap(~sport)
## Add medians or means to the plot... add in coin toss line

p2 <- ggplot(bigfour.last.ll, aes(Cumulative - Sequential)) + geom_histogram() + geom_vline(xintercept= 0) + 
...
p3 <- ggplot(bigfour.last.ll, aes(Cumulative - Sequential)) + geom_histogram() + geom_vline(xintercept= 0) + 
  facet_wrap(~sport, scales = "free") + ggtitle("Differences in game level log loss, cumulative - sequential")  + 
  xlab("Difference in log loss (sequential model predictions are better < 0)")


y1 <- bigfour.last.ll %>% filter(sport == "nhl") %>% select(Cumulative) %>% unlist()
y2 <- bigfour.last.ll %>% filter(sport == "nhl") %>% select(Sequential) %>% unlist()
#t.test(y1,y2,paired=TRUE)


@

\hl{Observed $p_{(MLB,s,k)ij}$'s boast lower average and median log-loss, which is likely accounted for the fact that these projections accurately assess which starting pitchers each team is using on that day, an important game-level factor not accounted for by Model IHA. Meanwhile, $cumulative$ and $sequential$ game-level probabilities match or exceed the accuracy shown in observed probabilities for each of the NBA, NFL, and NHL. Although these results do not suggest if an arbitrage opportunity exists (recall that sports books add a vig to each team's price), they do imply that both our team strength and home advantage estimates can be used to extract game-level projections, and that $sequential$ estimates, while slightly less accurate than $observed$ estimates, well approximate the chance of each team winning without using any compromising future information.}

\hl{$Note$: we could used paired tests to identify that LL accuracy is significantly better among cumulative fits. We could also resample games? }

\hl{$Note$: Possible plot: linearity of sequential and cumulative game probabilities. This shows that there is no noticeable difference, but not sure another figure is worth it}

\hl{$Note$: Possible plot: difference in game-level log loss, cumulative - sequential estimates. This shows that, in general, sequential models are slightly worse the cumulative models, and accounts for paired nature of the data}
