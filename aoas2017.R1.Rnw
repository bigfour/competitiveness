...
Using our posterior draws, we approximate the distribution of game-level probabilities between two randomly chosen teams using the following steps. \hl{Estimates from $cumulative$ IHA's are chosen given that they appear to more accurately link to game outcomes, and because our interest in assessing team equivalence is more retrospective than prospective.}  
...
<<bestwin-plot>>=
load(file.path("data", "bestwin.rda"))
