...
where $\text{logit}(.)$ is the log-odds transform \mike{and $\sum_{i=1}^{t_{q}} \theta_{(q,s,k)i} = 0$ for all $q,s,k$. As in} \cite{glickman1998state}, \mike{we center team strength estimates about 0 to ensure that our model is identifiable.} 
...
\mike{We next looked at $cumulative$ and $sequential$ game-level predicted probabilities, shown aggregated by sport in a scatter plot in the Appendix in Figure} \ref{fig:plotCompare}. \mike{For $sequential$ IHA fits, probabilities are estimated using team strengths from fits ending the week prior, to ensure that no future outcomes are used to inform predictions. Probabilities for both $sequential$ and $cumulative$ predictions use team-specific home advantage estimates. Overall, the links between each prediction type appear well represented by a linear fit, as no systematic differences are obvious. The correlations between predictions were greater than 0.97 in all sports; the NHL boasted the `weakest` link at 0.97. On a weekly basis, each NHL correlation was at least 0.92, each NBA correlation at least 0.95, and each NFL and MLB correlation at least 0.96. Apart from improvement in the link between $sequential$ and $cumulative$ NFL predictions as the season continued, there were no notable time trends (results not shown).} 

\mike{As a second check of predictive accuracy, we assess the game-level probabilities shown in Figure} \ref{plot.compare} \mike{by comparing to known game outcomes.} Table \ref{tab:ll} \mike{highlights the average and median log-loss in each sport comparing observed probabilities (our given $p_{(q,s,k)ij}$'s) versus estimated $p_{(q,s,k)ij}$'s imputed using both $cumulative$ and $sequential$ fits. Log-loss is a proper scoring rule for probabilistic predictions} \citep{gneiting2007strictly} and is a commonly used approach for assessing model accuracy in sports \citep{lopez2015building, ruiz2015generative, buttrey2016beating}.
...
<<BestWin, fig.height = 3.5, fig.width = 6, fig.cap="Cumulative distribution function (CDF) of 1000 simulated game-level probabilities in each league, for both neutral site and home games, with the better team (on average) used as the reference and given the home advantage.">>=
gg5 + theme(axis.text = element_text(size = 7))
@



Figure \ref{fig:BestWin} shows the cumulative distribution functions (CDFs) for each set of probabilities in each league. The median probability of the best team winning a neutral site game is highest in the NBA (67\%), followed in order by the NFL (64\%), NHL (57\%), and MLB (56\%). The spread of these probabilities are of great interest. Nearly every simulated MLB and NHL game played at neutral site is less than a 3:1 proposition with respect to the best team winning (75\%). Meanwhile, roughly 27\% of NBA and 20\% of NFL neutral site match-ups are greater than this 3:1 threshold. 
...
\mike{We propose a modified Bayesian state-space framework that can be used to estimate both time-varying strength variance parameters in order to better understand the underlying randomness in sporting organization, one which we implement on the NBA, NFL, NHL, and MLB.}
...
\mike{Although we restricted our analysis to North American professional sport, there are several ways our model generalizes to other competitions. As examples, comparisons between divisions of teams in the same organization (as in English soccer) or between the top leagues of the same sport (as in European soccer) would follow a similar framework to the one provided. Alternatively, in any sporting league, modeling the impact of structural changes (such as free agency, expansion or scoring system updates) would be straightforward to test by adding covariates to the model's provided. And team sports are not required for our model to apply; a similar structure could be implemented to assess the caliber of tennis players, which would be one approach for determining how their relative strength fluctuates with age. Finally, competitive balance questions within amatuer sport (for example, conferences in NCAA football) would follow a similar design.} 
...
<<plotCompare, fig.height=6.5, fig.cap="Comparison of game-level predictions made by $sequential$ and $cumulative$ state space fits. The light line reflects the line of best fit, while the dark line reflects equal probabilities.">>=
