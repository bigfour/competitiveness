...
ggplot(all.sport.both, aes(x = day.season, y = corr.day, lty = type, color = toupper(sport))) +
  ...
  scale_linetype_manual(labels = c("Point differential", "Sequential", "Win %"), 
                        values = c(2, 1, 3), "Type") +
...
aucs %>% 
  rename(`League ($q$)` = sport, 
         ...
         `AUC (sequential fits)` = auc.sequential)  %>%
  xtable(caption = "AUC values by sport", 
