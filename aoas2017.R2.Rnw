...
MLB &  -8587 & -8481  & -106.6 (37.5) \\
NBA & 6822  & 7188 & -366.2 (40.1) \\
NFL & 1100  & 1288 & -187.8 (24.4) \\
NHL & -18374  & -18128 & -244.7 (37.2)\\
\hline
\end{tabular}
}
...
\hl{We next address the fit of Models IHA and CHA by looking at the posterior predictive distribution of each. Formally, we assess whether combining Models IHA and CHA with draws from their respective posterior distributions can generate game-level data that roughly matches the observed data.} \ben{combining?}
...
Figure~\ref{fig:ppd-all} \hl{overlays each of Model IHA's 20 posterior predictive distributions of logit probabilities, shown in gray density curves, along with the observed distribution of logit probabilities, shown in red.  By and large, the observed distributions of logit probabilities are similar to the simulated distributions in each sport. In particular, the density in the tails of the posterior predictive distributions (reflecting probabilities near 0 or 1) do not show any serious departure from the observed distributions.} \ben{"meaningful" instead of "serious"?}

We purposefully use a lower bandwith for the density curves in Figure~\ref{fig:ppd-all} \hl{to highlight interesting discrepancies between the observed and predictive distributions. In the NBA and NFL, for example, the observed distribution is slightly lower that the simulated distributions with logit probabilities near 0 (i.e. both teams have a win probability of 0.5). This is likely occurring due to sportsbook preferences to set odds that are rounded to the nearest 5 (e.g. -105, -110, -155, etc.). As an example, there are 33 NFL games where the home team's boundary price is -185 (1.3\% of games), and there are 22 other prices that are observed for the home team in more 15 or more unique NFL games. Given that Model's CHA and IHA do not extract back to rounded prices for each team, it is not surprising that our posterior predictive distributions are smoother than the observed data. Similarly,} \cite{glickman1998state} \hl{found discrepancies between the observed distribution of point differential in the NFL and the posterior predictive distributions of point differential, on account of the increased likelihood of games ending with margins of victory of 3 or 7 in the NFL.  We believe that we are observing a similar phenomenon, but based on the increased likelihood of a sportsbook to assign odds rounded to the nearest 5 to games.    

Next, we compare the appropriateness of Models IHA and CHA for each team, as well as use posterior predictive distributions to contrast the two models. To do this, we calculate the average discrepancy between the mean posterior predictive distribution of each game and the observed game probability, averaged over home team for each model. These team level results are shown in} Figure~\ref{fig:ppd-model}. \hl{Discrepencies from Model CHA are shown in via circles, with arrows pointing towards the average discrepency for Model IHA. The color of the arrow (blue for yes, red for no) identifies whether or not, on average, Model IHA more closely matched the observed data than Model CHA. The dashed black line in each plot at 0 on the $x$-axis corresponds to home teams for whom, on average, the mean of the posterior predictive distribution matched that shown in our observed data. 

For most teams in most leagues, the posterior predictive distribution using Model IHA more appropriately reflects the observed data. In MLB, the two models perform nearly the same with the exception of the Colorado Rockies, whose home field advantage is underestimated when using Model CHA} (see Section~\ref{sec:home}).  \hl{In the NFL, Model IHA does not appear to offer a noticeable benefit, as discrepencies in Model IHA do not offer a marked improvement over those from Model CHA. Model IHA appears to offer a vast improvement over Model CHA in the NBA. As examples, observed home probabilities for Denver, Utah, and Golden State are underestimated using Model CHA, while those for Brooklyn, Detroit, New York, and Philadelphia, are, on average, overestimated. In the NHL, the posterior predictive distribution using Model IHA more closely matches the observed data for 27 of the 30 teams.}
...
Table \ref{tab:params} shows the mean and standard deviation of posterior draws for $\gamma_{q, \text{season}}$, $\gamma_{q, \text{week}}$, $\sigma_{q, \text{game}}$, $\sigma_{q, \text{season}}$, and $\sigma_{q, \text{week}}$ for each $q$. \hl{Before discussing results from these posterior distributions, it is important to recognize that each variance and autoregressive parameter is uniquely tied to each sports' relative logit scale. For example, the average posterior draw of $\gamma_{NBA, season}$ and $\gamma_{MLB, season}$ are both equal to 0.619, implying that relative to each league's distribution of team strengths, we can expect the same amount of reversion from one season to the next. However, given that there are larger gaps in the team strengths in the NBA, this corresponds to larger reversions in season-level strength when considered on an absolute scale.}
...
\hl{For each of the NBA, NFL, and NHL, the AUC and Brier metrics suggest that predictions made from sequential fits can closely approximate the observed game probabilities. However, our predictions yield a lower AUC and a higher Brier score in MLB, which is likely reflects our inability to account for each game's starting pitcher.} 
...
\mike{Not sure we address the revenue/fan engagement angle, so possibly worth dropping that sentence.}
...
\mike{Can we make each league the same color? When I printed in black and white, the MLB appeared to have the least parity}

\hl{Next, we construct a postseason tournament parity metric that acts as a pseudo-$R^2$. Let ${\bf F}=(F_{1},F_{2},\cdots,F_{z})$ be a $z$-dimensional random vector with the $d^{th}$ element indicating the tournament finish of the $d^{th}$ seed that depends on the vector of team strengths $\theta$.  That is, $F_d=1$, $F_d=2$, and $F_d=3$ indicates that the team seeded $d$ finished as tournament champions, finalists, and semi-finalists, respectively.  In a $z$-team tournament in which the higher seeded team always wins (i.e. the seeds determine the finish), the vector ${\bf F}$ is constant with $F_1=1$, $F_2=2$, $F_3=F_4=3$, $F_5=F_6=F_7=F_8=4$, etc, and, in general, $\Ex[F_d] = F_d =  \lceil \log_2{d} + 1 \rceil$ for $d = 1, 2, \cdots, z$.  In the other extreme, where the seed is irrelevant (i.e. All values of $\theta$ are equal and there is no home advantage), $\Ex[F_d] = \sum_{d=1}^z \frac{1}{z} \cdot \lceil \log_2{d} + 1 \rceil = f_z$ where $f_z$ is a constant that depends on $z$.  }

A pseudo-$R^2$ is then defined as follows:
$$
PostParity_z = \frac{(E[{\bf F}]-f_{z}J_{z})'(E[{\bf F}]-f_{z}J_{z})}{\sum_{d=1}^{z} (\lceil \log_2{d} + 1 \rceil - f_{z})^2}\,,
$$
...
\mike{One current issue: RegParity values of 1 indicate lots of parity, while PostParity values of 1 indicate no parity. We should consider addressing this}
