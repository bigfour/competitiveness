...
First, we collect the $k \in \{8, 16\}$ \hl{teams with the highest average team strength estimates over the last four weeks of each season, in each sport. We then seed (in descending order of team strength, irrespective of division or conference) and simulate 1000 postseason tournaments, in which each round consists of a best-of-7-game series, with the higher seed having the home field advantage in each round. The results shown in} Figure~\ref{fig:playoff-sims} \hl{(in the Appendix) confirm that the relationship between seed and tournament finish is strongest in the NBA and the NFL, and considerably weaker in MLB and the NHL. These findings accord with our regular season parity measures. }

\mike{Can we make each league the same color? When I printed in black and white, the MLB appeared to have the least parity}

\hl{Next, we construct a postseason tournament parity metric that acts as a pseudo-$R^2$. Let $F_i$ be a random variable indicating the tournament finish of the $i^{th}$ seed, where $F_i = 1$ for tournament champions, $F_i = 2$ for finalists, $F_i = 3$ for semi-finalists, and so on. In a $k$-team tournament in which the better team always wins, the seeds determine the finish, and thus $\Ex[F_i] = \lceil \log_2{i} + 1 \rceil = F_i^*$ for all $i$. On the other hand, if seeds are irrelevant then for all $i$, $\Ex[F_i] = \sum_{i=1}^k \frac{1}{k} \cdot F_i^* = f_k$, a constant that depends only on $k$. 
We define a pseudo-$R^2$}

\mike{Do we worry about using subscript $i$ again? We could change to subscript $d$ as to avoid readers from thinking that we are using $i$ for each team}
\greg{Yeah, we can't use $i$ again.}
\greg{So if went want to get really fancy, we could define the distribution of $F_i$ with a parameter that is the vector of $\theta$'s.  Then in the first case where the highers seeded teams always wins the theta's are all very far apart and when the seed doesn't matter is the setting where all the $\theta$'s are the exact same.}  
\greg{$F_{i}$ also depends on $k$.  So does $F^{\star}_{i}$.  Should we have a $k$ in the subscript for these?  What if we wrote the distribution of $F$ as a r.v. with two parameters: $k$ and $\theta$ where $\theta$ is a vector of length $k$.  }
\greg{Really $F$ should be defined as a multivariate distribution with dimension $k$ and parameter vector $\theta$.}
\greg{If we define this as a vector, you can write out the metric in a very nice form.  

$\frac{(E[{\bf F}]-f_{k}J_{k})'(E[{\bf F}]-f_{k}J_{k})}{\sum_{i=1}^{k} (\lceil \log_2{i} + 1 \rceil - f_{k})^2}$
}
