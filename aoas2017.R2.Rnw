...
\hl{Next, we construct a postseason tournament parity metric that acts as a pseudo-$R^2$. Let ${\bf F}=(F_{1},F_{2},\ldots,F_{z})$ be a $z$-dimensional random vector with the $d^{th}$ element indicating the round of tournament finish of the $d^{th}$ seed. (We note that $\mathbf{F}$ depends on the vector of team strengths $\theta$.)  That is, $F_d=1$ indicates that the team seeded $d$ finished as tournament champions, $F_d=2$ suggest they finished as finalists, $F_d=3$ that they finished as semi-finalists, and so on.  In a $z$-team tournament in which the higher seeded team always wins (i.e. the seeds determine the finish), the vector ${\bf F}$ is constant with $F_1=1$, $F_2=2$, $F_3=F_4=3$, $F_5=F_6=F_7=F_8=4$, etc., and in general, $\Ex[F_d] = F_d =  \lceil \log_2{d} + 1 \rceil$ for $d = 1, 2, \ldots, z$.  In the other extreme, where the seed is irrelevant (i.e., all values of $\theta$ are equal and there is no home advantage), $\Ex[F_d] = \sum_{d=1}^z \frac{1}{z} \cdot \lceil \log_2{d} + 1 \rceil = f_z$, where $f_z$ is a constant that depends on $z$.  }
