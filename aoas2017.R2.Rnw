...
We next assess whether or not using the assumptions of Models IHA and CHA with our posterior distribution draws can generate predictions of game probabilities (on the logit scale) that match the observed data. These distributions of predictions, termed posterior predictive distributions, can be used to both assess model fit and to compare different models.  
...
The distributions of implied probabilities on the logit scale are shown in Figure~\ref{fig:ppd-all} shows the results of 20 simulated sets of logit probabilties in gray lines over laid on one another, while the red line displays the density of the actual logit probabilities from the real data.  By and large, the observed distributions of logit probabilities are similar to the simulated distributions. In particular, the behaviour of the tails in the posterior predictive distributions across all four leagues is promising and does not show any serious departure from the real distribution of implied logit probabilities.
...
\ben{Need to change the text here if we decide to go with the new figure.}
...
%This is for the IHA model, correct? (ML: Yes, this is the IHA model)  
\begin{figure}[h]
...
Table \ref{tab:params} shows the mean and standard deviation of posterior draws for $\gamma_{q, \text{season}}$, $\gamma_{q, \text{week}}$, $\sigma_{q, \text{game}}$, $\sigma_{q, \text{season}}$, and $\sigma_{q, \text{week}}$ for each $q$. \hl{Before discussing results from these posterior distributions, it is important to recognize that these variance and autoregressive parameters are uniquely tied to each sports' relative logit scale. For example, the average posterior draw of $\gamma_{NBA, season}$ and $\gamma_{MLB, season}$ are both equal to 0.619, implying that relative to each league's scale, team strengths revert by roughly the same amount from one season to the next. However, given the differences} \ben{in what?} \hl{between the NBA and MLB, this amounts to a substantially larger reversion in the NBA on an absolute scale.}
...
\hl{Add in a note about how this somewhat validates our logit transform}
...
\noindent where we multiply by 2 in order to scale so that $0 \leq Parity_q \leq 1$, where 1 represents complete parity (every game a coin flip) and 0 represents no parity (every game outcome pre-determined). 
