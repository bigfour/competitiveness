...
\noindent where $\text{logit}(.)$ is the log-odds transform. We center team strength \hl{and individual home advantage} estimates about 0 to ensure that our model is identifiable (e.g., $\sum_{i=1}^{t_{q}} \theta_{(q,s,k)i} = 0$ for all $q,s,k$ and $\sum_{i^{\star}=1}^{t^{\star}_{q}} \alpha_{(q) i^{\star}} = 0$ )
