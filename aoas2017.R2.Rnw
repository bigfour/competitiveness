...
briers <- bigfour.last.ll %>%
  ...
  group_by(sport) %>%
  summarise(brier = mean(squared.score, na.rm = TRUE), brierS = mean(squared.scoreS, na.rm = TRUE))
...
aucs %>% 
  ...
  rename(`League ($q$)` = sport, 
         ...
         `Brier score (observed)` = brier, 
         `Brier score  (sequential fits)` = brierS)  %>%
