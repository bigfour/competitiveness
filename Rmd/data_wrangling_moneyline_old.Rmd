...
```{r, message=FALSE}
require(dplyr)
mlb <- read.csv("data/MLB-Data_moneyline.csv", stringsAsFactors = FALSE)
nba <- read.csv("data/NBA-Data_moneyline.csv", stringsAsFactors = FALSE)
nfl <- read.csv("data/NFL-Data_moneyline.csv", stringsAsFactors = FALSE)
nhl <- read.csv("data/NHL-Data_moneyline.csv", stringsAsFactors = FALSE)
```
...
ds <- semi_join(ds, games, 
                by = c("home_team", "gameTime", "visitor_nss" = "min_visitor_nss"))
...
```{r, eval=FALSE}
mline <- ds
save(mline, file = "data/mline.rda", compress = "xz")
```
