...
games <- ds %>%
  ...
  summarise(max_visitor_nss = max(visitor_nss))
ds <- semi_join(ds, games, by = c("home_team", "gameTime", "visitor_nss" = "max_visitor_nss"))
```
...
ds <- ds %>%
  ...
  filter(!(sport == "nba" & month(gameDate) < 10 & month(gameDate) > 5)) %>%
  filter(!(sport == "nfl" & month(gameDate) > 2 & month(gameDate) < 9)) %>%
  filter(!(sport == "nhl" & month(gameDate) < 9 & month(gameDate) > 5))
```
