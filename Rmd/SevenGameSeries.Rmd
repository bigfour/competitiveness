...
#Playoff teams
pteams <- read_csv("Data/playoff_teams.csv")

df.playoffs <- NULL
for (i in 1:4){
  for (season in 1:11){
    df.temp <- subset(pteams, sport == sports[i])
    pteams.temp <- df.temp$home_team[df.temp[,(season + 3)] == 1]
    df.playoffs.temp <- data.frame(t(combn(pteams.temp, 2)))
    df.playoffs.temp$sport <- sports[i]
    df.playoffs.temp$season <- 2005 + season
    df.playoffs <- rbind(df.playoffs, df.playoffs.temp)
  }
}

#write.csv(df.playoffs, "playoff.matchups.csv", row.names = FALSE)
...
```{r}
df.playoffs3 <- df.playoffs2 %>% group_by(1:n()) %>% mutate(prob.3 = games.out(prob.b.home, prob.b.road, 3), 
