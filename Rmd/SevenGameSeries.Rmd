...
p<- base_plot + aes(x = toupper(sport)) + 
  ...
  annotate("text", x = "MLB", y = 0.9, label = "GSW-2017") + 
  geom_text(data = gsw, aes(label = team.w, y = better.win.all), nudge_y = 0.01) + 
  geom_curve(data = gsw, aes(y = better.win.all, xend = "MLB", yend = 0.89), 
             curvature = -0.3, 
             arrow = arrow(length = unit(0.2, "cm"), ends = "first", type = "closed"))+ 
  annotate("text", x = 5, y = 0.9, label = "WSH-2017") + 
  geom_text(data = wsh, aes(label = team.w, y = better.win.all), nudge_y = 0.01) + 
  geom_curve(data = wsh, aes(y = better.win.all, xend = 5, yend = 0.89), 
             curvature = -0.3, 
             arrow = arrow(length = unit(0.2, "cm"), ends = "first", type = "closed"))
p
```
